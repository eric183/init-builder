...
class TemplateComponent extends Component {
    @computed get templateList (){
        return mobx.toJS(this.props.store.templateManage.templateList);
    }
    constructor(props){
        super(props);
    }
    ...
    componentDidMount() {
        //有缓存则不请求
        this.props.store.templateManage.getTemplateList(qs.parse(this.props.location.search.slice(1))); 
    }
    render(){
        return (
        ...
        return (
            <Modal
                ...
                <Form className="_platform" onSubmit={this.handleOk.bind(this)}>
                    ...
                    <FormItem {...formItemLayout} label="模板介绍（可选）">
                        {getFieldDecorator('username4', {
                            initialValue : this.props.formdata.username4
                        })(
                            <Input.TextArea autosize={{ minRows: 2, maxRows: 2 }} placeholder="请输入模板介绍"/>
                        )}
                    </FormItem>
                    <FormItem {...formItemLayout} label="模板分类">
                    ...
                    <FormItem {...formItemLayout} label="模板标签">
