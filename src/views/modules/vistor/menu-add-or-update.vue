<template>
    <el-dialog :title="!dataForm.id ? '新增' : '修改'" :close-on-click-modal="false" :visible.sync="visible">
        <el-form :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="dataFormSubmit()" label-width="80px" size="small">
            ...
            <el-form-item v-if="dataForm.type === 1" label="菜单路由" prop="url"> <el-input v-model="dataForm.url" placeholder="菜单路由"></el-input> </el-form-item>
            <el-form-item label="访问类型" prop="userType">
                <el-select v-model="dataForm.userType"> <el-option v-for="item in userTypeList" :key="item.value" :label="item.label" :value="item.value"></el-option> </el-select>
            </el-form-item>
...
export default {
    data() {
        ...
        return {
            ...
            userTypeList: [{value: 1, label: '物业人员'}, {value: 2, label: '企业管理员'}, {value: 3, label: '所有人员'}],
            dataRule: {
    ...
    methods: {
        init(id) {
            ...
            this.$http({
                ...
                .then(({data}) => {
                    this.menuList = treeDataTranslate(data.data.list, 'menuId');
                })
                .then(() => {
                    this.visible = true;
                    this.$nextTick(() => {
                ...
                .then(() => {
                    if (!this.dataForm.id) {
                        ...
                        // 修改
                        this.$http({
                            url: this.$http.adornUrl(`/v1/visitor/menus/` + this.dataForm.id),
                            method: 'get',
                        }).then(({data}) => {
                            console.log(data);
                            ...
                            this.dataForm.perms = data.data.permit;
                            this.dataForm.orderNum = data.data.oindex;
                ...
                if (valid) {
                    this.$http({
                        ...
                        data: this.$http.adornData({
                            ...
                            permit: this.dataForm.perms,
                            oindex: this.dataForm.orderNum,
