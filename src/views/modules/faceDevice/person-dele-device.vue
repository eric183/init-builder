<template>
    <el-dialog :visible.sync="visible" title="解绑设备">
        <el-table :data="dataList" style="width: 100%">
            ...
            <el-table-column fixed="right" label="操作" width="100">
                <template slot-scope="scope">
                    <el-button type="danger" size="mini" @click="deleDevice(scope.row.deviceId)">解绑</el-button>
                </template>
            </el-table-column>
        </el-table>
    </el-dialog>
...
export default {
    ...
    methods: {
        init(id) {
            ...
            this.visible = true;
            this.$http({
                url: this.$http.adornUrl4('/v1/face/users/' + this.userId + '/devices'),
                method: 'get',
            }).then(data => {
                data = data.data;
        ...
        deleDevice(deviceId) {
            ...
            this.$confirm(`确定移除设备吗?`, '提示', {
                confirmButtonText: '确定',
                cancelButtonText: '取消',
                type: 'warning',
            }).then(() => {
                this.$http({
                    url: this.$http.adornUrl4(`/v1/face/users/unBindingDevices`),
                    method: 'put',
