    ...
    <div class="mod-menu">
        ...
        <el-table :data="dataList" border style="width: 100%;">
            ...
            <el-table-column prop="type" header-align="center" align="center" label="类型">
                <template slot-scope="scope">
                    <el-tag v-if="scope.row.type === 1" size="small">模块</el-tag>
                    <el-tag v-else-if="scope.row.type === 2" size="small" type="success">目录</el-tag>
                    <el-tag v-else-if="scope.row.type === 3" size="small" type="info">菜单</el-tag>
                    <el-tag v-if="scope.row.type === 4" size="small">按钮</el-tag>
                </template>
            </el-table-column>
    ...
    methods: {
        ...
        deleteHandle(id) {
            this.$confirm(`确定对[id=${id}]进行[删除]操作?`, '提示', {
                ...
                .then(() => {
                    this.$http({
                        url: this.$http.adornUrl(`/v1/auth/sysMenus/` + id),
                        method: 'delete',
                    }).then(({data}) => {
                        this.getDataList();
                    });
