<template>
    <el-dialog :title="pageTitle" :close-on-click-modal="false" :visible.sync="dialogVisible">
        <el-table height="500" ref="multipleTable" :data="tableData" tooltip-effect="dark" style="width: 100%" @selection-change="handleSelectionChange">
...
export default {
    ...
    methods: {
        ...
        selectCheckDevices() {
            ...
            let that = this;
            this.$http({
                url: this.$http.adornUrl4('/v1/face/blacklistDevices/' + this.userId + '/devices'),
                method: 'GET',
                params: this.$http.adornParams(obj),
            }).then(data => {
                if (data && data.data.code === 200) {
        ...
        submmit() {
            if (this.multipleSelection && this.multipleSelection.length < 1) {
                ...
                this.$http({
                    url: this.$http.adornUrl4('/v1/face/blacklistDevices/bindingDevice'),
                    method: 'POST',
