...
<script>
import {commonFunc,commonFunc2,download} from '@/utils/resources/index.js';
...
export default {
    ...
    methods: {
        ...
        initTime() {
            ...
            this.dataForm.bookDate = commonFunc2(start.getTime());
        },
        getDataListFun() {
            this.pageNum = 1;
            this.getDataList();
        ...
        // 后端返回的数字，转换成中文
        bookDate(item) {
            return commonFunc2(item.bookDate);
        },
        payTime(item) {
            return commonFunc(item.payTime);
        },
        bookTime(item) {
