<template>
    <div class="mod-user">
        <p class="textright"><el-button size="small" v-if="isAuth('addwhite')" type="success" @click="addHandle()">新增</el-button></p>
        <el-table :data="dataList" border v-loading="dataListLoading" style="width: 100%;">
            ...
            <el-table-column header-align="center" align="center" v-if="isAuth('deletewhite')" min-width="80" label="操作">
                <template slot-scope="scope">
                    <el-button type="danger" plain size="mini" @click="isdelete(scope.row.whitelistId)">移除</el-button>
                </template>
            </el-table-column>
        </el-table>
        <el-pagination
...
export default {
    ...
    methods: {
        ...
        getDataList() {
            ...
            this.$http({
                url: this.$http.adornUrl('/v1/visitor/whitelists'),
                method: 'get',
        ...
        isdelete(id) {
            this.$confirm(`确定进行删除操作?`, '提示', {
                ...
                .then(() => {
                    this.$http({
                        url: this.$http.adornUrl('/v1/visitor/whitelists/' + id),
                        method: 'delete',
                    }).then(({data}) => {
                        this.getDataList();
                    });
