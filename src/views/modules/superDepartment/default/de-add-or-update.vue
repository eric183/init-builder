<template>
    <el-dialog :title="!dataForm.id ? '新增' : '修改'" :close-on-click-modal="false" :visible.sync="visible">
        <el-form :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="dataFormSubmit()" label-width="100px" size="small">
            ...
            <el-form-item label="部门负责人" prop="userId" v-if="dataForm.id">
                <el-select v-model="dataForm.userId"> <el-option v-for="item in userList" :key="item.userId" :label="item.realName" :value="item.userId"></el-option> </el-select>
            </el-form-item>
        </el-form>
    ...
    data() {
        return {
            ...
            dataRule: {
                ...
                userId: [{required: true, message: '必选', trigger: 'blur'}],
            },
            options: [],
            props: {
                value: 'departmentId',
            ...
            this.$http({
                ...
                .then(() => {
                    if (this.dataForm.id) {
                        // 修改
                        this.$http({
                            url: this.$http.adornUrl('/v1/pm/departments/' + this.dataForm.id + '/users/options'),
                            method: 'get',
                        }).then(({data}) => {
        ...
        dataFormSubmit() {
            this.$refs['dataForm'].validate(valid => {
                if (valid) {
                    ...
                    this.$http({
                        url: this.$http.adornUrl(this.dataForm.id == 0 ? `/v1/pm/departments` : `/v1/pm/departments/` + this.dataForm.id),
