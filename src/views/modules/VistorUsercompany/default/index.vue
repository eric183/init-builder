    ...
    <div class="mod-user">
        ...
        <el-form :inline="true" :model="dataForm" @keyup.enter.native="getDataListFun()" size="small">
            ...
            <el-form-item>
                ...
                <el-button type="success" @click="addVistor()">邀请访客</el-button>
        ...
        <el-table :data="dataList" border v-loading="dataListLoading" style="width: 100%;">
            ...
            <el-table-column header-align="center" align="center" width="240" label="操作">
                <template slot-scope="scope">
                    ...
                    <el-button type="success" plain size="mini" v-if="scope.row.status == 1" @click="switchChange(scope.row, 2)">通过</el-button>
                    <el-button type="danger" plain size="mini" v-if="scope.row.status == 1" @click="switchChange(scope.row, 3)">拒绝</el-button>
                </template>
            </el-table-column>
        </el-table>
        <el-pagination
...
import Add from './usercompany-add';
export default {
    data() {
        return {
            dataForm: {
    ...
    methods: {
        ...
        data() {
            ...
            this.startTime = commonFunc2(end.getTime());
            this.endTime = commonFunc2(end.getTime());
        },
        getDataListFun() {
            this.pageIndex = 1;
            this.getDataList();
        ...
        pdfPart() {
            if (this.dataForm.status) {
                ...
                window.location.href = this.$http.adornUrl(
                    '/v1/visitor/records/excels?pageSize=' + this.pageSize + '&pageNum=' + this.pageIndex + '&name=' + this.dataForm.name + '&startTime=' + this.startTime + '&endTime=' + this.endTime
                );
            }
        },
        //  导出当前条件下的全部
        pdfAll() {
            if (this.dataForm.status) {
                ...
                window.location.href = this.$http.adornUrl(
                    '/v1/visitor/records/excels?pageSize=' + this.totalPage + '&pageNum=' + this.pageIndex + '&name=' + this.dataForm.name + '&startTime=' + this.startTime + '&endTime=' + this.endTime
                );
            }
        },
        // 修改访客状态
        ...
        createdAt(item) {
            return commonFunc(item.arriveTime);
        },
    },
};
</script>
